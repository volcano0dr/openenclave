    ...
    oe_datetime_t* input_validation_time)
{
    ...
    sgx_qvl_supplemental_t* p_supplemental_data = NULL;
    ...
    if (result != OE_PLATFORM_ERROR)
    {
        ...
        // Untrusted QVL use GMT, convert it to local time
        p_supplemental_data = (sgx_qvl_supplemental_t*)supplemental_data;

        OE_CHECK(oe_gmt_to_localtime(
            &p_supplemental_data->earliest_expiration_date,
            &p_supplemental_data->earliest_expiration_date));
    ...
    // Pass quote verification with QVL/QvE
    // Try to get validity_from and validity_until from supplemental data
    if (result == OE_OK)
    {
        p_supplemental_data = (sgx_qvl_supplemental_t*)supplemental_data;

        OE_CHECK(oe_time_t_to_datetime(
            &p_supplemental_data->latest_issue_date, &validity_from));
        OE_CHECK(oe_time_t_to_datetime(
            &p_supplemental_data->earliest_expiration_date, &validity_until));
    }
